...
import { Loader2, PlusCircle, Trash2, Edit, Award, Briefcase, ArrowLeft, ArrowRight, Star, Building } from 'lucide-react';
...
export default function AdminSkillsExperiencePage() {
  ...
  }, [auth, auth.currentUser, fetchItems]);
  ...
  const handleSelectChange = (name: keyof SkillExperienceFormData, value: string) => {
    setFormData(prev => ({ ...prev, [name]: value }));
     if (name === 'type') { 
        setFormData(prev => ({ 
            ...prev, 
            company: value === 'skill' ? '' : prev.company, 
            role: value === 'skill' ? '' : prev.role, 
            dateRange: value === 'skill' ? '' : prev.dateRange, 
            category: value === 'experience' ? '' : prev.category,
            level: value === 'experience' ? undefined : prev.level || 'Intermediate'
        }));
    }
  };
      ...
      <div className="container py-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="name" name="name" value={formData.name} onChange={handleChange} required className="shadow-sm"/>
                    </div>
                </div>
                ...
                {formData.type === 'skill' && (
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Input id="category" name="category" value={formData.category} onChange={handleChange} placeholder="Örn: Frontend" className="shadow-sm"/>
                        </div>
                        <div className="space-y-2">
                ...
                {formData.type === 'experience' && (
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <Input id="company" name="company" value={formData.company} onChange={handleChange} className="shadow-sm"/>
                        </div>
                        <div className="space-y-2">
                        <Label htmlFor="role" className="text-sm font-medium text-foreground/90">Pozisyon/Rol</Label>
                        <Input id="role" name="role" value={formData.role} onChange={handleChange} className="shadow-sm"/>
                        </div>
                        <div className="space-y-2">
                        <Label htmlFor="dateRange" className="text-sm font-medium text-foreground/90">Tarih Aralığı</Label>
                        <Input id="dateRange" name="dateRange" value={formData.dateRange} onChange={handleChange} placeholder="Örn: Ocak 2020 - Halen" className="shadow-sm"/>
                        </div>
                    </div>
                  </>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="description" name="description" value={formData.description} onChange={handleChange} rows={3} required className="shadow-sm"/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="details" name="details" value={Array.isArray(formData.details) ? formData.details.join('\n') : ''} onChange={handleDetailsChange} rows={5} placeholder="Her bir detayı yeni bir satıra yazın." className="shadow-sm"/>
                </div>
        ...
        {isLoadingData ? (
          ...
          <div className="grid md:grid-cols-1 lg:grid-cols-2 gap-8">
            {items.map((item) => (
              <Card key={item.id} className="flex flex-col shadow-lg hover:shadow-xl transition-shadow duration-300 border rounded-lg">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge variant={item.type === 'skill' ? 'default' : 'secondary'} className="text-xs py-1 px-2.5">{item.type === 'skill' ? 'Beceri' : 'Deneyim'}</Badge>
                  </div>
                ...
                <CardFooter className="flex justify-end gap-2 border-t border-border/70 pt-4 pb-4 px-5 mt-auto">
                  <Button variant="outline" size="sm" onClick={() => handleEdit(item)} disabled={isSubmitting} className="shadow-sm hover:shadow-md transition-all">
